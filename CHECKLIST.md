...
2. Skopiuj pliki `.env.example` do `.env` w folderach `backend/` i `frontend/` oraz w głównym katalogu.
3. Uruchomienie standardowe (frontend, api, bazy):
   ```bash
   docker-compose up --build -d
   ```

4. Odczekaj ok. 30-60 sekund na inicjalizację bazy danych i statusy healthy.
...
3. **Weryfikacja limitów zasobów:**
   `docker stats` -> Wyświetla nałożone limity CPU i MEMORY dla kontenerów.
...
5. **Test trwałości danych (Persistence):**
...
6. **Test API (Endpoint /health):**
   `docker exec elearning-backend wget -qO- http://localhost:5000/health` -> Powinno zwrócić status health check passed.
...
- **Izolacja baz danych:** Baza Postgres, Redis i broker Mosquitto nie wystawiają portów na hosta. Ruch przechodzi wyłącznie przez backend.
...
- **Healthchecki:** Zdefiniowane dla wszystkich kluczowych usług (Postgres, Redis, Backend).
